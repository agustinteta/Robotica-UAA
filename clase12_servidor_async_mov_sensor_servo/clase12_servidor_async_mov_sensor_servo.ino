...
float distancia;
int velocidadMotor = 0; // Velocidad inicial del motor (0-255)
int anguloServo = 90; // Ángulo inicial del servo (0-180)
...
void setup() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    // Ruta POST para controlar el ángulo del servo
    server.on("/servo", HTTP_GET, [](AsyncWebServerRequest *request) {
      if (request->hasParam("valor")) {
        int angulo = request->getParam("valor")->value().toInt();
        if (angulo >= 0 && angulo <= 180) {
          anguloServo = angulo;
          miServo.write(anguloServo);
          char respuesta[30];
          snprintf(respuesta, sizeof(respuesta), "Angulo: %d", anguloServo);
          Serial.printf("Servo angulo: %d\n", anguloServo);
          request->send(200, "text/plain", respuesta);
        } else {
          request->send(400, "text/plain", "Error: rango invalido (0-180)");
        }
      } else {
        request->send(400, "text/plain", "Error: falta parametro 'valor'");
      }
